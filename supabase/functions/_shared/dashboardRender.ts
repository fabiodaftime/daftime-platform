...
export function renderDashboard(ctx: RenderCtx, plan: DashPlan): string {
  ...
  const chartsJs = `const CHARTS=${JSON.stringify(charts)};const PALETTE=${JSON.stringify(palette)};const PRIMARY=${JSON.stringify(primary)};const INK='${th.ink}',MUT='${th.muted}',GRID='${th.grid}',SURF='${th.surface}',DARK=${th.dark};const made={};
...
function opt(e,d){const ax={axisLine:{show:false},axisTick:{show:false},splitLine:{lineStyle:{color:GRID}},axisLabel:{color:MUT}};
 ...
 if(d.kind==='line')return{...BASE,animationDuration:850,legend:{show:d.series.length>1,bottom:0,icon:'circle',itemWidth:8,textStyle:{color:MUT}},xAxis:{type:'category',boundaryGap:false,data:d.labels,...ax,splitLine:{show:false}},yAxis:{type:'value',...ax},series:d.series.map(s=>({name:s.name,type:'line',data:s.data,smooth:true,symbol:'circle',symbolSize:6,showSymbol:false,lineStyle:{width:2.5,color:s.color},itemStyle:{color:s.color,borderColor:'#fff',borderWidth:2},areaStyle:d.series.length===1?{color:gr(e,s.color)}:undefined}))};
 if(d.kind==='bar')return{...BASE,animationDuration:800,xAxis:{type:'category',data:d.labels,...ax,splitLine:{show:false},axisLabel:{color:MUT,interval:0,hideOverlap:true}},yAxis:{type:'value',...ax},series:[{type:'bar',data:d.data.map((v,i)=>({value:v,itemStyle:{color:d.colors[i],borderRadius:[7,7,0,0]}})),barMaxWidth:54}]};
