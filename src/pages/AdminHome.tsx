// Plateforme staff Daftime : sidebar (implantations + Configuration) + clients de
// l'implantation sélectionnée. Legacy et nouveau modèle unifiés.
import { useEffect, useMemo, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { supabase } from '@/integrations/supabase/client';
import { Button } from '@/components/ui/button';
import { Search, Plus, Users, Upload, Building2, ChevronRight, Settings } from 'lucide-react';
...
export default function AdminHome() {
  ...
  const navItem = (icon: React.ReactNode, label: string, onClick: () => void) => (
    <button onClick={onClick} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-foreground hover:bg-muted transition">
      {icon} {label}
    </button>
  );
  ...
  return (
    <AppShell maxWidth="max-w-6xl">
      <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-6">
        {/* Sidebar : implantations + configuration */}
        <aside className="space-y-4">
          <nav className="rounded-xl border bg-card p-2 space-y-1">
            <div className="px-3 pt-1.5 pb-1 text-[11px] uppercase tracking-wide text-muted-foreground">Implantations</div>
            {LOCATIONS.map((l) => {
              const active = loc === l.key;
              return (
                <button key={l.key} onClick={() => setLoc(l.key)}
                  ...
                  <span className={`ml-auto text-xs ${active ? 'text-primary-foreground/70' : 'text-muted-foreground'}`}>{countOf(l.key)}</span>
                </button>
              );
            })}
          </nav>
        ...
        {/* Contenu : clients de l'implantation choisie */}
          ...
          {loading ? (
            ...
            <div className="text-center py-16 border border-dashed rounded-xl">
              ...
              <p className="text-muted-foreground text-sm">
                {q ? 'Aucun client ne correspond.' : `Aucun client à ${currentLoc.label}.`}
              </p>
            </div>
          ) : (
