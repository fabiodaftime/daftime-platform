...
export default function LandingEcommerce2({ advisor }: { advisor?: string } = {}) {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="px-5 pb-4">
        <div className="max-w-lg mx-auto">
          <div className="text-center">
            ...
            <p className="mt-3 text-muted-foreground">Pas un export de data. L’analyse d’un expert&nbsp;: ce qui cloche, et quoi faire — chiffré.</p>
          </div>
          ...
          <div className="mt-8 rounded-3xl border bg-card shadow-xl overflow-hidden">
            ...
            <div className="bg-primary text-primary-foreground px-5 py-5">
              <div className="text-[11px] uppercase tracking-widest text-primary-foreground/60">Le verdict · exemple</div>
              <p className="mt-2 text-[17px] font-bold leading-snug">« {VERDICT} »</p>
            </div>

            {/* les recommandations chiffrées — la force de proposition */}
            <div className="px-5 py-5">
              <div className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Ce qu’on te recommande, ce mois-ci</div>
            ...
            <div className="border-t bg-secondary/40 px-5 py-3 text-center text-[13px] font-medium text-muted-foreground">
      ...
      <section className="px-5 py-14">
        <div className="max-w-lg mx-auto">
          <h2 className="text-3xl font-extrabold tracking-tight text-center mb-8">Les questions qui reviennent.</h2>
          <div className="space-y-3">
            {FAQ.map((f) => (
              <details key={f.q} onToggle={(e) => { if (e.currentTarget.open) trackFaqOpen(f.q, 'ecommerce2'); }} className="group rounded-2xl border bg-card p-5">
                ...
                {Array.isArray(f.a)
                  ? <div className="mt-3 space-y-2.5">{f.a.map((p, i) => <p key={i} className="text-muted-foreground text-[15px]">{p}</p>)}</div>
