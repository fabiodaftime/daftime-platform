...
const FAQ = [
  ...
  { q: 'Je sais sortir le même dashboard avec l’IA. Pourquoi payer ?', a: 'Et tu as raison — la visualisation, c’est la couche facile. La valeur est en dessous : tracer tes flux économiques d’une source à l’autre, structurer ta data financière pour qu’elle dise vrai (une charge mal rattachée et ton point mort est faux), puis situer ta marge, ton ROAS et ton AOV face à ce qu’on voit passer sur +40 shops de ton secteur. Et être force de proposition : si le levier du mois c’est l’email, TikTok Shop ou Google Ads, on te le dit et on t’oriente vers le bon spécialiste. Le dashboard, tu le gardes — ce que tu paies, c’est cette lecture.' },
